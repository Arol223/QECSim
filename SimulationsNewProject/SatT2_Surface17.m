clear
clear GLOBAL
ngates = 760; % From MSc thesis, 250 gives highest p_th with max gain
...
T_2_spin = logspace(-3, 3, 6); % Logarithmically spaced values for spin T_2 between 1 ms and ~6 hours.
...
parfor i = 1:length(T_2_spin)
    ...
    [cnot,~,xgate,ygate,zgate,hadgate] = MakeGates(0,0,[7.7e-6*[1 1] 3.36e-6*[1 1 1 1]],0,1); %Gate objects
    [xgate,zgate,hadgate,ygate] = SetErrDiff(p_b_SQBG, p_p_SQBG, xgate, zgate, hadgate,ygate);
    ...
    [rtmp, ptmp] = CorrectionCycle(rtmp,'X',cnot,hadgate,xgate,zgate);
    [rtmp, ptmp] = CorrectionCycle(rtmp,'Z',cnot,hadgate,xgate,zgate);
...
title('Fidelity error as function of T_{2, spin}/T_{2, optical} for Surface17')
ylabel('\epsilon_{Fid}')
