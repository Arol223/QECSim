...
parfor i = 1:res
    ...
    if inc_rest
        ...
        rho.t_init = t_init(0.5*p,T_2_hf); 
        rho.t_ro = t_ro(5*p,T_2_hf); 
        t_dur_2QBG = t_init(0.5*p_cnot,T_2_hf);
        t_dur_SQBG = t_init(2*p_SQG,T_2_hf);
        c_phase_SQBG = 2*p_SQG;%DampCoeff(t_dur_SQBG, T_2_hf); % Phase damping coeff for SQBG
        c_phase_2QBG = 0.5*p_cnot%DampCoeff(t_dur_2QBG, T_2_hf); % -||- 2QBG
