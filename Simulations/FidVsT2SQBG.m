%% Short T2, T1 for Eu
T1 = 2e-3;
T2_short = logspace(-6,-4,20);
e_readout = 1e-3;
t_durs = 400e-9; % SQBG duration
t_dur_tot = [t_durs*4*[1 1] t_durs*[1 1 1 1]]; % 4 times longer for 2QBG
[cnot,cz,xg,yg,zg,hg] = MakeGates(T1,Inf,t_dur_tot,0,2);
fid_short = zeros(7,length(T2_short));
...
for i = 1:length(T2_short)
    cnot.T2 = T2_short(i);
    cnot.err_from_T();
    rtmp = cnot.apply(rho2);
    fid_short(2,i) = Fid2(psi2,rtmp);
end
